  ...
  for (const p of buys) {
    ...
    if ((st === "HIT" || st === "WAIT") && currentPrice <= p) {
      ...
      if (!canPlaceAnotherBuy()) {
        // mark as HIT (visual), but do not fill
        state.levelStates[k] = "HIT";
...
function tryExecuteSells(currentPrice) {
  ...
  // choose the lowest reached sell level (closest above anchor)
...
// =====================
// Dashboard HTML (Layout B) + Status Banner + Avg Entry / Breakeven
// =====================
...
function renderDashboard() {
  ...
  const breakeven = avgEntry; // paper trading: no fees yet
...
setInterval(keepAlive, KEEPALIVE_MS);    // Merge carefully (keep defaults if missing)
    state = { ...state, ...s };
